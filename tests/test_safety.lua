...
local parse = function(lines)
  return require("chroma-vault.config").parse_dump(lines)
end
...
T["is_encrypted"]["recognises a vault header"] = function()
  local r = with_lines({ "$ANSIBLE_VAULT;1.1;AES256", "3363..." }, function(buf)
    return require("chroma-vault")._test.is_encrypted(buf)
  end)
  eq(r, true)
end
...
T["context_dir"]["returns a string for a buffer"] = function()
  ...
  local r = with_lines({ "---" }, function(buf)
    return require("chroma-vault")._test.context_dir(buf)
  end)
  eq(type(r), "string")
end
...
local block_at = function(lines, lnum)
  return with_lines(lines, function(buf)
    return require("chroma-vault")._test.block_at(buf, lnum)
  end)
end
...
T["attach_writer"]["attaches one"] = function()
  ...
  require("chroma-vault").attach_writer(buf)
...
local vault = function()
  return require("chroma-vault")._test
end
...
T["encrypt_selection"]["refuses without a range"] = function()
  ...
  local notified = capture_notify(require("chroma-vault").encrypt_selection)
...
T["encrypt_selection"]["refuses when the range is empty"] = function()
  local notified = capture_notify(function()
    require("chroma-vault").encrypt_selection({ range = 0, line1 = 1, line2 = 1 })
  end)
  eq(notified:find("needs a range") ~= nil, true)
end
...
local differs = function(a, b)
  return require("chroma-terraform")._test.context_differs(a, b)
end
...
local overriding = function()
  return require("chroma-aws")._test.overriding_credentials()
end
...
T["aws spawn"]["a lookup that cannot start is reported, not raised"] = function()
  ...
  local ok, err = pcall(require("chroma-aws").whoami)
...
T["starting environment"] = new_set({
  hooks = {
    pre_case = function()
      ...
      require("chroma-aws")._test.forget_initial_environment()
    end,
    post_case = function()
