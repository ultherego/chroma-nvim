-- Where this plugin is allowed to put a plan file.
--
-- A saved plan is not ciphertext. It quotes the variable values the run
-- resolved, which for infrastructure code routinely means secrets, so it gets
-- the same treatment as one: the XDG runtime directory, which the
-- specification requires to be owned by the user, to have mode 0700, and not
-- to survive logout.
--
...
-- /tmp without a word.
--
-- This file is deliberately part of terraform.nvim rather than shared with the
-- rest of the configuration. See DECISIONS.md, "Each is a plugin, not a file
-- in a shared namespace": the module has to be liftable into its own
-- repository unchanged, and a shared helper is exactly what makes that
-- impossible. ansible-vault.nvim carries its own copy of this policy for the
-- same reason.
...
---Owner and permissions of a directory that is about to hold a plan.
---@param path string
